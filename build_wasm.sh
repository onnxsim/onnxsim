...
NODE_CMAKE=(-DONNXSIM_WASM_NODE="$WITH_NODE_RAW_FS")

mkdir -p "$BUILD_DIR"
cd "$BUILD_DIR"
emcmake cmake -DONNX_CUSTOM_PROTOC_EXECUTABLE=$PROTOC -DCMAKE_BUILD_TYPE=Release -DCMAKE_CXX_FLAGS="-sNO_DISABLE_EXCEPTION_CATCHING" "${NODE_CMAKE[@]}" "${ORT_WEB_CMAKE[@]}" ..
