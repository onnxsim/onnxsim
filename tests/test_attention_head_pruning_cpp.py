...
``_slice_or_gather_head_bias`` for this one chain family), but this port
remains DELIBERATELY NARROWER than pruning.py's own matcher in every other
respect: no decomposed RoPE/Q-K-norm pass-through, no ``Einsum``-based QK^T/AV
product, no packed-QKV-then-``Split`` producer, and no true-MQA fast path.
Every one of those shapes still declines to match in this C++ port (never
...
# -- exactly mirroring pruning.py's own behavior. This port remains
# deliberately narrower than pruning.py's own `_find_decomposed_gqa_chains`
# in every other respect: no decomposed RoPE/Q-K-norm pass-through, no
