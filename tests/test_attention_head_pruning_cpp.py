...
RoPE/Q-K-norm pass-through, no ``Einsum``-based QK^T/AV product. Every one of
...
# every one of those shapes simply declines to match here (never mis-sliced)
# and is still fully handled by pruning.py's own pure-Python
# `apply_attention_head_pruning`, so this tenth family is NOT yet aliased.
# A packed-QKV-then-`Split` producer (MatchDecomposedPackedQkvProducer) AND
# a true-MQA fast path (ApplyOneDecomposedGqaChain's own `is_mqa` handling)
# ARE both matched/applied here now, mirroring pruning.py's own
# `_match_decomposed_packed_qkv_producer`/`is_mqa` handling exactly -- see
# each's own comment in structured_pruning_entry.cpp.
